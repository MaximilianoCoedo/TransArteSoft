...
{
    public class BRServicios
    {

        public static int solicitarServicio(int idUsuario, string fechaSalida, string horaSalida, string fechaDestino, string horaDestino, string descripcionServicio)
        {

            try
            {
                BOServicios bo = new BOServicios();
                int id = bo.Add(idUsuario, fechaSalida, horaSalida, fechaDestino, horaDestino, BRSeguridad.encriptar(descripcionServicio.ToUpper()));
                Int32 nVerificacion = calcularDVH(id);
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarServicios("TransArte_Servicios");
                BRBitacora.registrarEvento("Se solicitarServicio id: " + id, BeanException.Critico);
                return id;
            }
            catch (Exception ex)
            {
        ...
        public static DataSet getConfirmarServicios()
        {
            try
            {
                BOServicios bo = new BOServicios();
                return bo.GetByN("Solicitados", "");
            }
            catch (Exception ex)
            {
                throw;
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: TransArte_Servicios id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_Servicios"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_Servicios");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_Servicios ", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_Servicios"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            ...
            try
            {
                ...
                {
                    BRDVVertical.actualizarServicios("TransArte_Servicios");
                    BRBitacora.registrarEvento("Rev. Consist. Vertical Tabla TransArte_Servicios:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_Servicios:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
