    ...
    public class BREstadoServicio
    {
        private static Int32 calcularDVH(int id)
        {
            ...
            try
            {
                ...
                BOEstadoServicio bo = new BOEstadoServicio();
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: TransArte_EstadoServicio id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_EstadoServicio"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_EstadoServicio");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_EstadoServicio", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_EstadoServicio"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            ...
            try
            {
                ...
                if (!nIntVertical.Equals(nSuma))
                {
                    BRDVVertical.actualizarEstadoServicio("TransArte_EstadoServicio");
                    BRBitacora.registrarEvento("Rev. Consist. Vertical Tabla TransArte_EstadoServicio:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_EstadoServicio:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
