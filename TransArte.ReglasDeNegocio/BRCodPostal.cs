...
{
    public class BRCodPostal
    {

        public static DataSet buscarPorCodPostal(string codPostal)
        {
            try
            {
                BOCodPostal bo = new BOCodPostal();
                return bo.GetByN("codPostal", codPostal);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: TransArte_CodPostal id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_CodPostal"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_CodPostal");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_CodPostal", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_CodPostal"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            try
            {
                ...
                if (!nIntVertical.Equals(nSuma))
                {
                    BRDVVertical.actualizarCodPostal("TransArte_CodPostal");
                    BRBitacora.registrarEvento( "Rev. Consist. Vertical Tabla TransArte_CodPostal:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_CodPostal:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
