...
{
    public class BRMantenimientos
    {
        public static int agregarMantenimiento(int idVehiculo, string descripcion, string kilometros, string fechaMantenimiento)
        {
            try
            {
                BOMantenimientos bo = new BOMantenimientos();
                int id = bo.Add(idVehiculo, descripcion, kilometros, fechaMantenimiento);
                Int32 nVerificacion = calcularDVH(id);
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarMantenimientos("TransArte_Mantenimientos");
                BRBitacora.registrarEvento("Se dio de alta el Mantenimiento id: " + id, BeanException.Critico);
                return id;
            }
            catch (Exception ex)
            {
        ...
        public static void modificarMantenimiento(int id, int idVehiculo, string descripcion, string kilometros, string fechaMantenimiento)
        {
            try
            {
                BOMantenimientos bo = new BOMantenimientos();
                bo.UpdateBy("IdMantenimiento", id, idVehiculo, descripcion, kilometros, fechaMantenimiento);
                Int32 nVerificacion = calcularDVH(id);
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarMantenimientos("TransArte_Mantenimientos");
                BRBitacora.registrarEvento("Se modifico el Mantenimiento id: " + id, BeanException.Critico);
            }
            catch (Exception ex)
            {
        ...
        public static void bajaMantenimiento(int id)
        {
            try
            {
                BOMantenimientos bo = new BOMantenimientos();
                ...
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarMantenimientos("TransArte_Mantenimientos");
                BRBitacora.registrarEvento("Se dio de baja el Mantenimiento id: " + id, BeanException.Critico);
            }
            catch (Exception ex)
            {
        ...
        public static DataSet getMantenimientos()
        {
            try
            {
                BOMantenimientos bo = new BOMantenimientos();
        ...
        public static DataSet getMantenimientos(int idVehiculo)
        {
            try
            {
                BOMantenimientos bo = new BOMantenimientos();
                return bo.GetByN("IdVehiculo", idVehiculo);
            }
            catch (Exception ex)
            {
                throw;
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: Mantenimientos id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_Mantenimientos"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_Mantenimientos");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_Mantenimientos", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_Mantenimientos"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            ...
            try
            {
                ...
                {
                    BRDVVertical.actualizarMantenimientos("TransArte_Mantenimientos");
                    BRBitacora.registrarEvento("Rev. Consist. Vertical Tabla TransArte_Mantenimientos:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_Mantenimientos:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
