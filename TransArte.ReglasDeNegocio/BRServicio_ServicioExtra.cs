...
{
    public class BRServicio_ServicioExtra
    {

        
        public static int agregarServicioExtra(int idServicio, int idServicioExtra)
        {

            try
            {
                BOServicio_ServicioExtra bo = new BOServicio_ServicioExtra();

                int id = bo.Add(idServicio, idServicioExtra);
                Int32 nVerificacion = calcularDVH(id);
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarServicio_ServicioExtra("TransArte_Servicio_ServicioExtra");
                BRBitacora.registrarEvento("Se agrego TransArte_Servicio_ServicioExtra id: " + id, BeanException.Critico);
                return id;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: TransArte_Servicio_ServicioExtra id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_Servicio_ServicioExtra"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_Servicio_ServicioExtra");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_Servicio_ServicioExtra", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_Servicio_ServicioExtra"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            ...
            try
            {
                ...
                {
                    BRDVVertical.actualizarServicio_ServicioExtra("TransArte_Servicio_ServicioExtra");
                    BRBitacora.registrarEvento("Rev. Consist. Vertical Tabla TransArte_Servicio_ServicioExtra:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_Servicio_ServicioExtra:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
