...
{
    public class BRObrasDeArte
    {

        public static int agregarObraDeArte(int idServicio, string nombre, string descripcion, string ancho, string alto, string largo, string peso, string valor)
        {
            try
            {
                BOObrasDeArte bo = new BOObrasDeArte();
                int id = bo.Add(idServicio, nombre.ToUpper(), descripcion.ToUpper(), ancho, alto, largo, peso, valor);
                Int32 nVerificacion = calcularDVH(id);
                actualizarDVH(id, nVerificacion);
                BRDVVertical.actualizarObrasDeArte("TransArte_ObrasDeArte");
                BRBitacora.registrarEvento("Se agrego TransArte_ObrasDeArte id: " + id, BeanException.Critico);
                return id;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    {   //La suma de verificación Horizontal no corresponde. 
                        BRBitacora.registrarEvento("Consist. Horizontal incorrecta en Tabla: ObrasDeArte id = " + ds.Tables[0].Rows[j]["id"], BeanException.Critico);
                        usuario.addIntegridad(new BeanIntegridad(identificador, "TransArte_ObrasDeArte"));
                    }
                    nSuma = nSuma + nVerificacion;
                    j = j + 1;

                } // Fin Recorro todas las filas de la tabla
                Int32 nIntVertical = BRDVVertical.consistenciaVertical("TransArte_ObrasDeArte");
                if (!nIntVertical.Equals(nSuma))
                {
                    BRBitacora.registrarEvento("Consist. Vertical incorrecta en Tabla: TransArte_ObrasDeArte", BeanException.Critico);
                    usuario.addIntegridad(new BeanIntegridad("", "TransArte_ObrasDeArte"));
                    return false;
                }
                else
                    return true;
        ...
        public static Boolean corregirIntegridad()
        {
            ...
            try
            {
                ...
                {
                    BRDVVertical.actualizarObrasDeArte("TransArte_ObrasDeArte");
                    BRBitacora.registrarEvento("Rev. Consist. Vertical Tabla TransArte_ObrasDeArte:OK!", BeanException.Critico);
                }
                BRBitacora.registrarEvento("Rev. Consist. Horizontal Tabla TransArte_ObrasDeArte:OK!", BeanException.Critico);
                return true;
            }
            catch (Exception ex)
            {
